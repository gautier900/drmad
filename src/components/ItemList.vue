<template>
  <div>
    ...
    <CheckedList :data="filterViruses" :itemCheck="true" :checked="checked" :itemButton="{ text: 'Ajouter au panier', show: true }"
      :listButton="{ text: 'Voir virus cochés', show: true }" :itemAmount="true" @item-button-clicked="ajoutPanier"
      @checked-changed="changeSelection" @list-button-clicked="toutAjouter"></CheckedList>
  </div>
</template>
...
const ajoutPanier = ((LinkIndex) => {
    // ajouter l'item dans le panier + sa quantité
  alert(filterViruses.value[LinkIndex].name)
});
...
const toutAjouter = (() => {
  // ajouter tous les virus au panier selon leur quantité
  alert("Virus ajoutés au panier")
})
